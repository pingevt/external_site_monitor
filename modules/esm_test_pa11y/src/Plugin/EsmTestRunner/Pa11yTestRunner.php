...
namespace Drupal\esm_test_pa11y\Plugin\EsmTestRunner;
 ...
 *
 * @EsmTestRunner(
 *   id = "pa11y_test_runner",
 *   test_type = "pa11y_test",
 *   test_result_type = "pa11y_test_result"
 * )
 */
class Pa11yTestRunner extends EsmTestRunnerBase implements EsmTestRunnerInterface, ContainerFactoryPluginInterface {
  ...
  public function runTest($test) {
    ...
    if ($config->get('github_token')) {
      ...
      $callback_url = Url::fromRoute('esm_test_pa11y.pa11y_results', [], ['absolute' => TRUE])->toString();
      ...
      $this->loggerFactory->get('esm:pa11y')->debug("Attempting to Call Github Action: <pre>" . print_r([
        $config->get('api_url'),
        $post_data,
      ...
      $this->loggerFactory->get('esm:pa11y')->debug("Response Code: @code", ["@code" => $response_code]);

      $errno = curl_errno($ch);
      if ($errno ) {
        $error_message = curl_strerror($errno);
        $this->loggerFactory->get('esm:pa11y')->error("cURL erro ({@errno}): @err", [
          "@errno" => $errno,
          "@err" => $error_message,
        ]);
      }
      ...
      $views[] = [
        ...
        "group" => [
          [
            ...
            'view' => views_embed_view("pa11y_test_result", "chart_base", $test->id(), $url_field_data['uri']),
          ],
          [
            '#attributes' => [
              'class' => ["o-url-result--table"],
            ],
            'view' => views_embed_view("pa11y_test_result", "base", $test->id(), $url_field_data['uri']),
          ],
        ],
      ];
    }
