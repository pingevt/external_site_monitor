...
namespace Drupal\esm_test_pa11y\Controller;
...
class Api extends ControllerBase implements ContainerInjectionInterface {
   ...
   */
  public function pa11yResults(Request $request) {
    ...
    // Grab config.
    $config = $this->configFactory->get('esm_test_pa11y.settings');
    ...
    $this->loggerFactory->get('esm:pa11y')->debug("Recieved Pa11y results. <pre>" . print_r($body_json, TRUE) . "</pre>");
    ...
    foreach ($body_json->results as $i => $result) {
      ...
      $this->loggerFactory->get('esm:pa11y')->debug("Attempting to find Reports. <pre>" . print_r($report_id_arr, TRUE) . "</pre> " . $result->url);
      ...
      if (!empty($reports)) {
        ...
        // Add files.
        $html_filename = explode(".pa11yci/", $result->htmlPath)[1];
        $json_filename = explode(".pa11yci/", $result->jsonPath)[1];
        ...
        $this->loggerFactory->get('esm:pa11y')->debug("File target_html_file. <pre>" . print_r($target_html_file, TRUE) . "</pre>");
        $this->loggerFactory->get('esm:pa11y')->debug("File target_json_file. <pre>" . print_r($target_json_file, TRUE) . "</pre>");
        ...
        if (file_exists($target_html_file)) {
          ...
          $this->loggerFactory->get('esm:pa11y')->error("File DNE. <pre>" . print_r($target_html_file, TRUE) . "</pre>");
        }
        if (file_exists($target_json_file)) {
          ...
          $this->loggerFactory->get('esm:pa11y')->error("File DNE. <pre>" . print_r($target_json_file, TRUE) . "</pre>");
        }
        ...
        // Log error.
        $this->loggerFactory->get('esm:pa11y')->error("No reports found. " . print_r($result, TRUE));
      }
    }
  ...
  private function createFile($file_uri, $entity) {
    ...
    $file_usage->add($file, 'esm_test_pa11y', 'result', $entity->id());
